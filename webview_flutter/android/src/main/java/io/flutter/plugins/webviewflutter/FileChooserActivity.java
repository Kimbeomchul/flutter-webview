...
public class FileChooserActivity extends Activity {
    ...
    private static final SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyyMMdd_HHmmss"); // List of Uris that point to files where there MIGHT be the output of the capture. At most one of these can be valid
    ...
    private Uri getTempUri(String format) {
        ...
        return FileProvider.getUriForFile(this, getApplicationContext().getPackageName() + ".generic.provider", file);
    }
        ...
        if (requestCode == FILE_CHOOSER_REQUEST_CODE) {
            ...
            if (resultCode == Activity.RESULT_OK) {
                if (data != null && (data.getDataString() != null || data.getClipData() != null)) {
                    if (data.getDataString() != null) { // single result from file browser OR video from camera
                        Uri localUri = copyToLocalUri(data.getData());
                        if (localUri != null) {
                            fileChooserFinishedIntent.putExtra(EXTRA_FILE_URIS, new String[]{localUri.toString()});
                        }
                    } else if (data.getClipData() != null) { // multiple results from file browser
                ...
                } else { // image result from camera (videos from the camera are handled above, but this if-branch could handle them too if this varies from device to device)
                    for (Uri captureOutputUri : potentialCaptureOutputUris) {
                        try { // just opening an input stream (and closing immediately) to test if the Uri points to a valid file
